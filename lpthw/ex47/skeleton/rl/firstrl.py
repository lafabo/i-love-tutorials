	...
	for i in range(num_monsters):
		...
		if not is_blocked(x, y):
			if libtcod.random_get_int(0, 0, 100) < 80: # 80% chance to get the ork
				monster = Object(x, y, 'o', 'ork', libtcod.desaturated_green, blocks=True)
			else:
...
def player_move_or_attack(dx, dy):
	...
	# coordinates for move or attack
	x = player.x + dx
	y = player.y + dy

	# try to find attackable object here
	target = None
	for object in objects:
	...
	# attack if target found, move if not
	if target is not None:
		print 'The ' + target.name + ' laughs at your attempt to attack him'
	else:
		player.move(dx, dy)
		fov_recompute = True
